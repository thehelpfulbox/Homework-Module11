...
def show_all_contacts(*page):
    print(f"page:{page}")
    if contacts:
        for record in contacts.iterator(int(page[0])):
            print(record)
    else:
...
def hello(*result):
    return """How can I help you?
        - To add new contact or update the existing contact, type: <add name number>
        - To add or change birthday in the existing contact, type: <birthday name birth_date>
        - To find contacts by any parameter, type: <find parameter>
        - To show number of days remaining to birthday of the contact, type <when name>
        - To show the contact details of the person, type: <phone name>
        - To show all contacts in the directory, type: <show all #_of_records_per_page> 
